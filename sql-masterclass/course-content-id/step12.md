...
# Langkah 12 - Pertanyaan Studi Kasus Akhir
...
Untuk menyelesaikan seluruh studi kasus kripto kita - mari kita hitung query yang persis sama dengan yang baru saja kita buat untuk data Nandita dengan seluruh dataset kita yang mencakup semua mentor!

## Membuat Tabel Dasar

> Buat tabel ringkasan `mentor_performance` yang mencakup metrik berikut untuk setiap anggota dan ticker:
...
* Profitabilitas Aktual (nilai portofolio akhir + pendapatan penjualan kotor - biaya pembelian - biaya penjualan) / jumlah investasi awal
* Profitabilitas Teoretis (nilai portofolio akhir tanpa penjualan - biaya pembelian) / jumlah investasi awal
...
Pastikan untuk melihat hasil dari temp table dengan menjalankan query `SELECT` sebelum mengerjakan pertanyaan-pertanyaan berikut!

```sql
SELECT * FROM mentor_performance;
```

**Peringatan - ini adalah tabel yang sangat sangat lebar!!!**
...
> Mentor mana yang memiliki profitabilitas aktual terbesar untuk setiap ticker?
...
> Mentor mana yang memiliki perbedaan terbesar antara profitabilitas aktual vs teoretis untuk setiap ticker?
...
> Berapa total jumlah pendapatan penjualan yang dihasilkan oleh semua mentor untuk setiap region? (gabungan BTC dan ETH)
...
> Berapa rata-rata profitabilitas aktual untuk setiap region untuk setiap ticker?
...
> Mentor mana yang memiliki investasi awal terbesar di setiap ticker?
